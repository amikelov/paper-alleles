...
cat oscar-coreyw-downsampling_depth.tsv | parallel --colsep '\t' -j3 'sample_depth_k=$(echo "{2}/1000" | bc); \
	...
	$(find ~/paper_alleles/raw0/watson/noumi/primerTrimmed -type f -name "{1}*R2*.fastq") ); \
	input_file="${input_files}"; \
	input_file=$(echo "$input_file" | sed "s|/qualityModified/primerTrimmed||g"); \
	output_file="${input_file}_${sample_depth_k}K.fastq"; \
	seqtk sample -s1707 "$input_file" "{2}" > "$output_file"'
	
	
cat oscar-coreyw-downsampling_depth.tsv | parallel --colsep '\t' -j3 'sample_depth_k=$(echo "{2}/1000" | bc); \
	input_files=( $(find ~/paper_alleles/raw0/watson/5race -type f -name "{1}*R1*.fastq") \
	$(find ~/paper_alleles/raw0/watson/noumi/primerTrimmed -type f -name "{1}*R1*.fastq") ); \
	echo $input_files; \
	input_file="${input_files}"; \
	input_file=$(echo "$input_file" | sed "s|/qualityModified/primerTrimmed||g"); \
	output_file="${input_file}_${sample_depth_k}K.fastq"; \
...
## [MiXCR]{.underline} upstream pipeline for full and downsampled data using minimal library (=1 allele per V/J gene)

## Immcantation upstream pipeline for full and downsampled data using minimal library (=1 allele per V/J gene)
...
### Run TIgGER w/ benchmarking  for 5'RACE downsampled
...
purrr::keep(bench_5race_downsampled, function(x) any(class(x) == "character"))
bench_df<-purrr::keep(bench_5race_downsampled, function(x) all(class(x) != "character"))
...
donorsWithRepseq<- dir("~/paper_alleles/benchmarking/raw/repseq/",
                       pattern="R1") %>%
...
## Add to iGenotyper expression information from Rep-Seq
...
ls *V*fasta|parallel 'repseqio -V alleles fromFasta -f --taxon-id 9606 --species-name hs --species-name homsap --species-name hsa --chain IGH --name-index 0 --gene-type V --gene-feature VRegion {} {.}.json'

ls *J*fasta|parallel 'repseqio -V alleles fromFasta -f --taxon-id 9606 --species-name hs --species-name homsap --species-name hsa --chain IGH --name-index 0 --gene-type J --gene-feature JRegion {} {.}.json'
...
ls *V*json |cut -f 1 -d "_" | parallel -j 10 --line-buffer 'echo {}; J=$(ls {}_*J.json); repseqio -V alleles  merge --replace-duplicate-genes -f  {}_*V.json /home/amikelov/ref/watson_hg38/watsonlab_lib.v.json /home/amikelov/ref/watson_hg38/watsonlab_lib.d.json $J /home/amikelov/ref/watson_hg38/watsonlab_lib.j.json /home/amikelov/paper_alleles/benchmarking/igenotyper/mixcr/watsonlab_lib.c.json {}_iGenotyper.json; repseqio  -V alleles  inferPoints -f -g VRegion -g JRegion  {}_iGenotyper.json {}_iGenotyper.json'
```
...
#repseqio throws exception on  IGHV4-61*novel-3 and IGHJ1*novel-1 which are both extremely truncated iGenotyper hits. need to remove them manually
# add to debug tsv  IGHV4-61*novel-3  IGHJ1*novel-1
echo -e 'IGHV4-61*novel-3\tFR1 contains a stop codon' >> debug.tsv
echo -e 'IGHJ1*novel-1\ttruncated' >> debug.tsv
```

### Remove all genes which didn't pass debug and write cleaned fasta
...
# 5'RACE
parallel --line-buffer  -j 4 'id=$(ls ../../raw/repseq/{}*R1* | cut -f 1-2 -d "_"|sed "s/.*\\///"); donor=$(echo $id|cut -d "_" -f 2);echo $id;echo $donor; mixcr -Xmx22g -V alleles analyze generic-bcr-amplicon-umi  -f --species hs --library ${donor}_iGenotyper --rna --rigid-left-alignment-boundary --floating-right-alignment-boundary C --tag-pattern "^(R1:*) \ ^(UMI:N{"12"})GTACG{1:10}(R2:*)"   --assemble-clonotypes-by VDJRegion --split-clones-by C ../../raw/repseq/${id}_{{R}}.fastq  $id' ::: SRR23446568 SRR23446580 SRR23446567 SRR23446566 SRR23446585 SRR23446587 SRR23446564 SRR23446565 SRR23446588 SRR23446578 SRR23446563 SRR23446579 SRR23446569 SRR23446577 SRR23446571 SRR23446582 SRR23446576 SRR23446572 SRR23446573 SRR23446583 SRR23446574 SRR23446584 SRR23446589 SRR23446586 SRR23446581 SRR23446590 SRR23446575
...
ls /home/amikelov/paper_alleles/benchmarking/raw/repseq/noumi/*R1.fastq | \
    cut -f 1-3 -d "_"| \
    parallel -S 3/bober0.milaboratories.com -S 3/bober1.milaboratories.com \
    ...
    echo {}; echo $donor;
    /software/mixcr/mixcr -Xmx22g analyze generic-bcr-amplicon \

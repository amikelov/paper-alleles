...
ls *clns |parallel -j 46 --line-buffer \
  "/usr/bin/time -f '\t%e real,\t%U user,\t%S sys,\t%K avmem,\t%M maxmem' \
  -o findAlleles/{.}.time.log mixcr findAlleles -f \
  -t 10 \
  --no-clns-output \
  --export-library findAlleles/{.}.fasta \
  --export-alleles-mutations findAlleles/{.}.tsv \
...
makeblastdb -parse_seqids -dbtype nucl -in /usr/local/share/igblast/fasta/imgt_human_ig_v.fasta \
    -out /usr/local/share/igblast/database/imgt_human_ig_v
# D segment database

makeblastdb -parse_seqids -dbtype nucl -in /usr/local/share/igblast/fasta/imgt_human_ig_d.fasta \
    -out /usr/local/share/igblast/database/imgt_human_ig_d
# J segment database

makeblastdb -parse_seqids -dbtype nucl -in /usr/local/share/igblast/fasta/imgt_human_ig_j.fasta \
    -out /usr/localshare/igblast/database/imgt_human_ig_j

realpath  *[2s].fasta | parallel -j 88 --line-buffer  'docker exec igb changeo-igblast -r /usr/local/share/germlines/imgt/human/vdj -b /usr/local/share/igblast -s {} -n {} -p 3 -o /home/amikelov/paper_alleles/benchmarking/immcantation_fasta/igblast'
```
...
cd /home/amikelov/paper_alleles/benchmarking/immcantation/downsampled/5race
realpath  *_collapse-unique.fastq | parallel -j 88 --line-buffer  'docker exec igb changeo-igblast -r /usr/local/share/germlines/imgt/human/vdj -b /usr/local/share/igblast -s {} -n {.} -p 3 -o /home/amikelov/paper_alleles/benchmarking/immcantation/downsampled/igblast'

cd /home/amikelov/paper_alleles/benchmarking/immcantation/downsampled/noumi
realpath  *_assemble-pass.fastq | parallel -j 88 --line-buffer  'docker exec igb changeo-igblast -r /usr/local/share/germlines/imgt/human/vdj -b /usr/local/share/igblast -s {} -n {.} -p 3 -o /home/amikelov/paper_alleles/benchmarking/immcantation/downsampled/igblast'
...
ls  /home/amikelov/paper_alleles/raw0/watson/5race/*R1*.fastq | parallel \
    ...
    echo {}; echo $donor;
    mixcr -Xmx22g analyze generic-bcr-amplicon-umi -f \
    --species hs --rna \
    --rigid-left-alignment-boundary \
    --floating-right-alignment-boundary C \
    --library ${donor}_iGenotyper \
...
```{r}
tiggerStats<-
  dir("~/paper_alleles/benchmarking/immcantation_fasta",
    pattern = "_db-pass.tsv",
    full.names = T) %>% 
  c(dir("~/paper_alleles/benchmarking/immcantation/downsampled/noumi",
    pattern = "_db-pass.tsv",
    full.names = T),
    dir("~/paper_alleles/benchmarking/immcantation/downsampled/5race",
    pattern = "_db-pass.tsv",
    full.names = T)) %>% 
  lapply(function(flnm){
    db<-read_tsv(flnm) %>% 
      ...
      mutate(fileName=flnm,
             ...
             depth=str_extract(flnm,"10K|25K|50K|100K|500K"))
  }) %>% bind_rows() %>% 
...
```{r}
mixcrStats<-
  dir("~/paper_alleles/benchmarking/mixcr",
    pattern = ".clones_IGH.tsv",
    full.names = T,
    recursive = T) |> 
  lapply(function(flnm){
    db<-read_tsv(flnm,show_col_types = F) %>% 
      separate_rows(allVHitsWithScore,sep=",") %>% 
...
mixcr_allele_files<-c(dir("~/paper_alleles/benchmarking/mixcr/noumi/findAlleles",
                          pattern = ".tsv",
                          full.names = T),
                     dir("~/paper_alleles/benchmarking/mixcr/5race/findAlleles",
                          pattern = ".tsv",
                          full.names = T),
                     dir("~/paper_alleles/benchmarking/mixcr/downsampled/noumi/findAlleles",
                          pattern = ".tsv",
                          full.names = T),
                     dir("~/paper_alleles/benchmarking/mixcr/downsampled/5race/findAlleles",
                          pattern = ".tsv",
                          full.names = T))
...
alleles_mixcr<-read_tsv(mixcr_allele_files,id="fileName") %>% 
  filter(str_detect(geneName,"IGHV|IGHJ")) %>% 
  mutate(donor = str_extract(fileName,"(SC|W)-[0-9]{1,3}"),
         depth = str_extract(fileName,"10K|25K|50K|100K|500K"),
...
```{r}
fastaFromMixcr<-c(dir("~/paper_alleles/benchmarking/mixcr/noumi/findAlleles",
                          pattern = ".fasta",
                          full.names = T),
                     dir("~/paper_alleles/benchmarking/mixcr/5race/findAlleles",
                          pattern = ".fasta",
...
fastaFromTigger<-c(
  dir("~/paper_alleles/benchmarking/immcantation_fasta/tigger",
...
all_alleles<-lapply(alleleFastas,function(fasta){
  ...
  donorAlleleSeq <- tibble(alleleName=names(donorAlleleSeq) %>%
                           ...
                           donor = str_extract(id, "(SC|W)-[0-9]{1,3}") %>%
                             str_remove("_.*"),
                           tool = str_extract(fasta,"tigger|mixcr"), 
                           depth = str_extract(fasta,"10K|25K|50K|100K|500K"))%>%           
...
protocol_donor<-tibble(
  donor=c(dir("~/paper_alleles/raw0/watson/5race",
              pattern="*_R1.fastq"),
          dir("~/paper_alleles/raw0/watson/noumi",
              pattern="*_R1.fastq")) |> 
    str_extract( "(SC|W)-[0-9]{1,3}") |>
    str_remove("_.*"),
  protocol=c(rep("5race",27),rep("noumi",46))
)
...
alleleComparison<-
  ...
  mutate(geneName=ifelse(is.na(geneName.y),as.character(geneName.x),geneName.y),.before=alleleSeq.x) %>% 
  ungroup() %>%
  dplyr::rename(
    donor=donor.y,
...
alleleComparison %<>%
  ...
  mutate(matchStatus=
           case_when(
             ...
             tool=="mixcr" & enoughInfo == F ~ "notEnoughInfo",
...
# Comparison divided by TYPE (5'RACE/ Vmix) of data
...
comparisonStats<-
  ...
  mutate(
    ...
    depth=factor(depth,levels=c("full", "500K","100K","50K","25K","10K")),
...
```{r}
igenotyper_muts<-read_tsv(dir("~/paper_alleles/benchmarking/igenotyper/mixcr/mutations",
                              pattern=".tsv",
                              full.names = T),id = "filename")

igenotyper_muts <- igenotyper_muts |> 
  rename(shm=`nMutationsRate{FR1Begin(6):FR3End}`) |> 
  mutate(donor=str_remove_all(filename,".*\\/|_IGH.tsv|SRR.*?_"))

shmStats <- igenotyper_muts |> 
  filter(shm!="region_not_covered") |> 
  mutate(shm=as.numeric(shm)) |> 
  group_by(donor) |> 
  summarise(meanSHM=mean(shm))
...
g1<-comparisonStats %>%
    ...
    mutate(matchStatus = factor(matchStatus, levels = c("falseNegative", "match", "falsePositive")),
           fraction=ifelse(donor=="W-43",0.1153846153,fraction),
           fraction=ifelse(donor=="W-70",0.1136363636,fraction),
           fraction=ifelse(donor=="W-107",0.125,fraction)) %>%
    left_join(shmStats, by = "donor") %>%
    ggplot(aes(y = fraction, x = meanSHM)) +
    geom_point() +
    #facet_wrap(~tool) +
    stat_cor(aes(label = paste(..r.label.., ..p.label.., sep = "~`,`~")), method = "pearson",
             label.x.npc = "middle", label.y.npc = "top") +
    theme_bw()+
   labs(y="Fraction of\nmissed alleles",x = "Mean SHM level in a sample")
...
g2<-comparisonStats %>%
    ...
    mutate(matchStatus = factor(matchStatus, levels = c("falseNegative", "match", "falsePositive"))) %>%
    left_join(shmStats, by = "donor") %>%
    ggplot(aes(y = N, x = meanSHM)) +
    geom_point() +
    #facet_wrap(~tool) +
    stat_cor(aes(label = paste(..r.label.., ..p.label.., sep = "~`,`~")), method = "pearson",
             label.x.npc = "middle", label.y.npc = "top") +
    theme_bw()+
   labs(y="# potential false\npositive calls",x = "Mean SHM level in a sample")
...
facet_labels <- c(mixcr = "MiXCR", tigger = "TIgGER")

g3<-comparisonStats  %>% 
    left_join(protocol_donor,by="donor") |> 
    filter(
        totalClonotypes>3000,
        matchStatus %in% c("match"),
        tool=="mixcr") %>%   #,"tooFewClonesInRepseqNotEnoughInfo")) %>% 
    mutate(matchStatus=factor(matchStatus,levels=c("falseNegative","match","falsePositive")),
           fraction=ifelse(donor=="W-43",1-0.1153846153,fraction),
           fraction=ifelse(donor=="W-70",1-0.1136363636,fraction),
           fraction=ifelse(donor=="W-107",1-0.125,fraction)) %>% 
    ggplot(aes(y=fraction,x=protocol))+
    geom_boxplot(fill="#198020")+
 #   facet_grid( ~ tool , labeller = labeller(tool = facet_labels))+
 # geom_point(position = position_dodge(width=0.75))+
  scale_x_discrete(labels = c("5race" = "With UMIs", "noumi" = "Without UMIs")) +
    stat_compare_means(method = "kruskal", label.x = 1.35, label.y = 0.0)+
  theme_bw()+
  labs(y="Fraction of alleles\nsupported by long reads")
...
g4<-comparisonStats  %>% 
  ...
  filter(
    ...
    matchStatus %in% c("falsePositive"),
        tool=="mixcr") %>%   #,"tooFewClonesInRepseqNotEnoughInfo")) %>% 
  mutate(matchStatus=factor(matchStatus,levels=c("falseNegative","match","falsePositive"))) %>% 
  ggplot(aes(y=N,x=protocol))+
    geom_boxplot(fill="#198020")+
  #  facet_grid( ~ tool , labeller = labeller(tool = facet_labels))+
  #geom_point(position = position_dodge(width=0.75))+
  scale_x_discrete(labels = c("5race" = "With UMIs", "noumi" = "Without UMIs")) +
    stat_compare_means(method = "kruskal", label.x = 1.5, label.y = 7.5)+
  theme_bw()+
...
figS4<-plot_grid(
  plot_grid(g3,g4,ncol = 2,labels=c("A","B")),
  plot_grid(g1,g2,nrow = 2,labels=c("C","D")),
  rel_heights = c(0.7,1.2),nrow=2
  )
                  

ggsave(filename = "plots/figS4.pdf", plot= figS4,device = "pdf",
       width = 8,height =6,units = "in",bg="white")
ggsave(filename = "plots/figS4.png", plot= figS4,device = "png",
...
```{r}
grateful::cite_packages(out.dir = ".",citation.style = "apa-6th-edition")
```

#FigS3 Time benchmarking plot

```{r}
donorsForBench<-comparisonStats |> filter(tool=='mixcr',depth=='full',totalClonotypes>3000) |> pull(donor) |> unique() 

names(bench_full)<-dir("~/paper_alleles/benchmarking/immcantation_fasta",
                  pattern="_db-pass.tsv",
                  full.names = T) |> 
  str_extract("S?[WC]-[0-9]{1,3}")

figS3<-bench_full[names(bench_full) %in% donorsForBench] |> 
    keep( ~ is_tibble(.x)) |> bind_rows(.id="donor") |> 
    mutate(total_time.tigger = as.numeric(as_bench_time(total_time))) |> 
  left_join(
    c(
      dir("~/paper_alleles/benchmarking/mixcr/5race/findAlleles", pattern = "time.log",full.names = T),
      dir("~/paper_alleles/benchmarking/mixcr/noumi/findAlleles", pattern = "time.log",full.names = T)
    ) |> 
      read_tsv(id='filename',col_names = c('empty','real','user',"sys","avmem","maxmem")) |> 
      mutate(donor=filename |>  str_extract("S?[WC]-[0-9]{1,3}"),
             total_time.mixcr=str_remove(real," real,") |> as.numeric()) |> 
      select(donor,total_time.mixcr),
    by="donor"
  ) |> 
  pivot_longer(cols=starts_with("total_time."),names_to = "tool") |> 
  mutate(tool=str_remove(tool,"total_time.")) |> 
  ...
  scale_x_discrete(labels=c(mixcr="MiXCR",  tigger="TIgGER"))+
  labs(y="Time (s)", x= "Tool")

ggsave(filename = "plots/figS3.pdf", plot= figS3,device = "pdf",
       width = 6,height =4,units = "in",bg="white")
ggsave(filename = "plots/figS3.png", plot= figS3,device = "png",
       width = 6,height = 4,units = "in",bg="white")
```
...
fig25k_match <- comparisonStats  %>%
  filter(totalClonotypes > 3000,
         matchStatus %in% c("match"),
         depth == "25K") %>%
  ggplot(aes(y = fraction, x = depth, fill = tool)) +
  geom_boxplot(outlier.shape = NA,
  ...
  facet_wrap(~ matchStatus,
             labeller = labeller(
               inStartRef = c(`FALSE` = "Absent in starting reference library",
                              `TRUE` = "Present in starting reference library"),
               matchStatus = c(
                 match = "Alleles supported by long reads",
                 falseNegative = "Alleles missed",
...
ggsave(filename = "plots/Fig25K_matched.png", plot= fig25k,device = "png",
               width = 6,height = 4,units = "in",bg="white")
ggsave(filename = "plots/Fig25K_matched.png", plot= fig25k,device = "png",
               width = 6,height = 4,units = "in",bg="white")
```
...
ggplot(aes(y = fraction, x = tool, fill = tool)) +
  ...
  stat_summary(fun.data = give.n,
               geom = "text",
               size = 3) +
  stat_summary(fun.data = give.median,
               geom = "text",
               size = 3) +
  scale_x_discrete(labels = c("MiXCR", "TIgGER")) +
  theme(
...
comparisonStats <-
  ...
  complete(matchStatus, depth, nesting(donor, tool), fill = list(N = 0)) %>%
  group_by(donor, tool) %>%
  mutate(nAllelesTotal = sum(N[matchStatus == "match" |
                                 matchStatus == "falseNegative"])) %>%
  mutate(fraction = N / nAllelesTotal) %>%
  left_join(stats %>% select(donor, totalReads:readsUsedInClonotypes))
comparisonStats  %>%
  filter(totalClonotypes > 3000,
         matchStatus %in% c("match")) %>%   #,"tooFewClonesInRepseqNotEnoughInfo")) %>%
  mutate(matchStatus = factor(
    matchStatus,
    levels = c("falseNegative", "match", "falsePositive")
  )) %>%
  ggplot(aes(y = fraction, x = tool, fill = tool)) +
  geom_boxplot(outlier.shape = NA) +
  facet_grid(~ matchStatus,
...
comparisonStats %>%
  ...
  left_join(shmStats, by = "donor") %>% View("shm")
...
comparisonStats %>%
  ...
  left_join(shmStats, by = "donor") %>% View()
comparisonStats %>%
  filter(totalClonotypes > 3000,
         matchStatus %in% c("match"),
         tool == "mixcr") %>%
...
populationFreqs <- alleles_db %>%
  ...
  mutate(Ethnicity = factor(
    str_replace(ethnicity, "Hispanic/Latino", "Hisp./Lat.") |>
...
color_fun = colorRamp2(c(0, 0.25, 0.75, 1),
                       c("#FAFAB4", "#8DDA7F", "#337E90", "#1C0F5C"))
...
allHeatmaps <-
  lapply(allVGenes, function(v) {
    ...
    alleleFeatures <- populationFreqs %>%
      ...
      mutate(
        novelCol = ifelse(isNovel, mi_bright[4], "black"),
        novelFont = ifelse(isNovel, "bold", "plain")
      )
    alleleFeatures <-
      alleleFeatures[match(
        colnames(heatmapMatrixFreqs[, -1]) %>% str_remove("_.*"),
        alleleFeatures$alleleNumber
      ), ]
    g <- Heatmap(
      heatmapMatrixFreqs[, -1] ,
      show_row_dend = F,
      show_column_dend = F,
      col = color_fun,
      na_col = "white",
      ...
      column_names_gp = gpar(
        col = alleleFeatures$novelCol,
        fontface = alleleFeatures$novelFont,
        fontsize = 8
      ),
      column_names_rot = 0,
      ...
      cell_fun = function(j, i, x, y, width, height, fill) {
        if (!is.na(heatmapMatrixCounts[, -1][i, j])) {
          if (heatmapMatrixFreqs[, -1][i, j] > 0.5) {
            font_color = "white"
          } else {
            font_color = "black"
          }
          grid.text(heatmapMatrixCounts[, -1][i, j],
                    x,
                    y,
                    gp = gpar(fontsize = 10, col = font_color))
        }
      },
